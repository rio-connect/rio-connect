...
class AdminProfileCollection extends BaseProfileCollection {
   ...
   */
  define({ email, password, name, phoneNo }) {
    if (Meteor.isServer) {
      ...
      if (!user) {
        ...
        const profileID = this._collection.insert({ email, name, userID: this.getFakeUserId(), role });
        const userID = Users.define({ username, role, password, name, phoneNo });
