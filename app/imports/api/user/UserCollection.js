...
class UserCollection {
   ...
   */
  define({ username, role, password, name, phoneNo }) {
    // if (Meteor.isServer) {
    ...
    if (_.get(Meteor, 'settings.public.development', false)) {
      const userID = Accounts.createUser({ username, password: credential, role, name, phoneNo });
      Roles.addUsersToRoles(userID, [role]);
      console.log(`Defining ${role} ${username} with password ${credential}`);
      return userID;
    }
    // Otherwise define this user with a Meteor login and default password.
    console.log(`Defining ${role} ${username} with password ${credential}`);
    const userID = Accounts.createUser({ username, password: credential, role, name, phoneNo });
    Roles.addUsersToRoles(userID, [role]);
    return userID;
  ...
  /**
   * Returns true if user is referenced by other "public" entities. Specifically if user is in a club.
   * Used to determine if user can be deleted.
   * @param user
   * @return {boolean}
   */
  isReferenced(user) {
    return Clubs.find({ owner: user }).fetch().length > 0;
  }
