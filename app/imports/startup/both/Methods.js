...
/** Creates a new club in the Clubs collection, and also updates ProfilesProjects and ProjectsInterests. */
Meteor.methods({
  'Clubs.add'({ name, description, image, type, members, ownerMail }) {
    const owner = Meteor.user().username;
    Clubs.collection.insert({ name, description, image, type, members, owner, ownerMail });
    ProfileClubs.collection.remove({ club: name });
    ClubInterests.collection.remove({ club: name });
    if (type) {
      type.map((interest) => ClubInterests.collection.insert({ club: name, interest }));
    } else {
      throw new Meteor.Error('At least one interest is required.');
    }
    if (members) {
      members.map((member) => ProfileClubs.collection.insert({ club: name, profile: member }));
    }
  },
});
